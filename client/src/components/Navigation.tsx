...
const ServicesDropdown = memo<ServicesDropdownProps>(
  ({ isOpen, onToggle, currentRoute, onNavigate }) => (
...
const UserMenu = memo<UserMenuProps>(({ user, isOpen, onToggle, onLogout, isMobile = false }) => (
  <div className="relative">
    ...
    >
      <div
        className={`${isMobile ? 'w-6 h-6' : 'w-8 h-8'} rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center ${isMobile ? 'mr-1' : 'mr-2'}`}
      >
      ...
      {isOpen && (
        ...
        >
          <div className={`${isMobile ? 'mb-3' : 'mb-4'}`}>
            <div
              className={`flex items-center space-x-${isMobile ? '2' : '3'} ${isMobile ? 'mb-2' : 'mb-2'}`}
            >
              <div
                className={`${isMobile ? 'w-8 h-8' : 'w-12 h-12'} rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center`}
              >
              ...
              <div>
                ...
                <p
                  className={`text-gray-400 ${isMobile ? 'text-xs' : 'text-sm'} ${isMobile ? '' : 'truncate'}`}
                >
                  {user.email}
                </p>
              </div>
...
export function Navigation() {
  ...
  const isActive = useCallback(
    (path: string) =>
      path === '/' ? currentRoute === '/' || currentRoute === '' : currentRoute === path,
    [currentRoute]
  );
