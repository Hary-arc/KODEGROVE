...
export function ResponsiveCardShowcase({
  showControls = true,
  maxCards,
}: ResponsiveCardShowcaseProps) {
  ...
  return (
    <div className="space-y-8 lg:space-y-12">
      ...
      <div className="text-center space-y-4">
        ...
        >
          Cards that adapt seamlessly across all screen sizes with fluid layouts and intelligent
          content management.
        </motion.p>
      </div>
        ...
        >
          {[
            ...
            <div key={label} className="flex flex-col items-center space-y-2">
              <div
                className={`w-12 h-12 sm:w-16 sm:h-16 glass rounded-2xl flex items-center justify-center text-purple-400 hover:text-white hover:bg-purple-500/20 transition-all duration-300 cursor-pointer`}
              >
              ...
              >
                <ResponsiveCardHeader responsive={true}>
                  <div className="flex items-center space-x-3 sm:space-x-4">
                    <div
                      className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br ${card.gradient} flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}
                    >
          ...
          {displayCards.map((card, index) => {
            ...
            return (
              <div key={`masonry-${card.id}`} className="break-inside-avoid mb-6 lg:mb-8">
                ...
                >
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${card.gradient} opacity-10 group-hover:opacity-20 transition-opacity duration-500 rounded-xl`}
                  />

                  <ResponsiveCardHeader responsive={false}>
                    <div className="flex items-center justify-between">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br ${card.gradient} flex items-center justify-center`}
                      >
                  ...
                  <ResponsiveCardContent spacing="md">
                    <ResponsiveCardTitle
                      size="lg"
                      className="text-white group-hover:text-purple-300 transition-colors duration-300 mb-3"
                    >
                      {card.title}
                    </ResponsiveCardTitle>
            ...
            return (
              ...
              >
                ...
                <div className="w-full lg:w-80 flex-shrink-0 relative">
                  <div
                    className={`w-full h-48 lg:h-64 rounded-2xl bg-gradient-to-br ${card.gradient} flex items-center justify-center relative overflow-hidden`}
                  >
                ...
                <div className="flex-1 flex flex-col justify-between space-y-4 lg:space-y-6">
                  <div>
                    <ResponsiveCardTitle
                      size="xl"
                      className="text-white group-hover:text-purple-300 transition-colors duration-300 mb-4"
                    >
                      {card.title}
                    </ResponsiveCardTitle>
                    <ResponsiveCardDescription className="text-gray-300 group-hover:text-gray-200 transition-colors duration-300 text-lg leading-relaxed">
                      {card.description} This adaptive layout automatically switches between
                      vertical and horizontal arrangements based on screen size, providing optimal
