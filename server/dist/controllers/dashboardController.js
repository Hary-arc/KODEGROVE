import { projectStore, invoiceStore, supportTicketStore, clientAnalyticsStore, ClientAnalytics, } from '../models/index.js';
...
export const getUserAnalytics = async (req, res) => {
    try {
        ...
        const analytics = await clientAnalyticsStore.findAll((a) => a.userId === userId &&
...
async function generateNotifications(userId, projects, invoices, tickets) {
    const notifications = [];
    // Project milestones
    ...
    const dueSoonInvoices = invoices.filter(i => i.status === 'pending' && new Date(i.dueDate) <= threeDaysFromNow && i.dueDate >= today);
